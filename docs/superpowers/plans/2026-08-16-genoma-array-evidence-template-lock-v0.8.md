...
O plano original foi preservado em `docs/history/v3.3/` porque descreve uma baseline normativa substituída. Ele permanece disponível para proveniência, mas não é uma fonte operacional ativa.
...
- `POST-DEPLOYMENT PASS` só pode ser registrado depois do merge e do Production Witness no SHA exato de `main`, com `15/15`, `critical_failures == 0` e `post_deployment_status == "PASS"`.
