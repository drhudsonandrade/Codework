...
- restrict bypasses to the minimum actor set supported by the repository plan/settings;
...
If GitHub cannot express an actor restriction for this user-owned private repository, the workflow's exact-SHA idempotency guard plus force-push/deletion protection remains mandatory, and that limitation must be recorded rather than represented as enforced.
