...
`tests/test_ci_optimization_contract.py` separately pins the Draft → local validation → Ready for Review → required CI flow as exact visible lines in the PR template, so negated or substring-only variants cannot satisfy the contract.
...
The documentation-language detector reuses the repository-wide technical-term vocabulary from `config/code_language_policy.json`, adds structural prose terms used by Markdown documentation, normalizes common plural/gender inflections back to known Portuguese terms, detects Portuguese diacritics regardless of letter case, and applies a dependency-free fallback to otherwise unknown all-ASCII prose. The reviewed proper names `Tupí` and `Rondônia` are filtered only when they tokenize as exact standalone names; prefixes or suffixes attached to either name remain subject to analysis, so TitleCase is not a general bypass. Known inflections can be sufficient on their own; the unknown-ASCII fallback combines common Portuguese function words with characteristic verb and nominal endings and requires at least two independent hints. Exact policy contract literals and canonical ruleset filenames are removed longest-first before prose analysis. Email addresses are stripped as address syntax, and only a dotted `.com` suffix is neutralized for bare-domain false positives; dotted identifiers remain tokenized and test-covered. Positive fixtures cover accented prose, uppercase accented prose, dotted Portuguese identifiers, one-token headings, inflected ASCII prose, and multiple unknown all-ASCII sentences, while English, email, and domain negative controls protect against broad false positives. Markdown syntax is not an exemption: blockquotes, fenced-code comments, and prose inside backticks are scanned like other text. The whole-line preservation set contains **45 exact lines** with reviewed reasons `historical_quote` or `localized_example`; exactly **5 Portuguese inline-code literals** are separately enumerated, including one `safety_pattern`. Tests require every line/literal exception to occur exactly once, carry an allowed reason, and still trigger the detector if its explicit exemption is removed.
