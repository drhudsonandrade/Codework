...
The documentation-language detector reuses the repository-wide technical-term vocabulary from `config/code_language_policy.json`, adds structural prose terms used by Markdown documentation, detects lowercase words carrying Portuguese diacritics, and applies a dependency-free multi-signal heuristic to all-ASCII prose. That fallback combines common Portuguese function words with characteristic verb and nominal endings and requires at least two independent hints before classifying an otherwise unknown ASCII line; positive fixtures include `Isso ocorreu ontem.` and `Precisamos corrigir outros problemas.`, while English negative controls protect against broad suffix-only false positives. Markdown syntax is not an exemption: blockquotes, fenced-code comments, and prose inside backticks are scanned like other text. The only whole-line bypasses are **29 exact lines** enumerated in the fixture with one of two reviewed reasons, `historical_quote` or `localized_example` (1 ancestry line, 1 editorial CLI example, 7 pharmacogenomic lines, 4 SNP-array CLI-example lines, and 16 target-registry historical lines). In addition, exactly **2 Portuguese inline-code literals** are explicitly enumerated and justified: the historical ancestry formula `48 + linha × indivíduos` and the localized review-tier label `só 1★`. Tests require every line/literal exemption to occur exactly once, carry an allowed reason, and still trigger the detector if its explicit exemption is removed.
