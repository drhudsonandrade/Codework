...
def _orientation(row: dict[str, str], schema: str, qc: dict[str, Any]) -> tuple[str, str]:
    ...
    # This explicit boolean is the trust boundary. Legacy text, truthy integers and
    # malformed verdicts must be re-generated by QC rather than receiving an implicit pass.
    verified = inputs.get("strand_evidence_verified") is True
    forward = normalized_strand in FORWARD_STRANDS and verified
    ...
    *,
    ...
    try:
        ...
        for row in reader:
            ...
            overlap_status = status if schema.startswith("harmonized") else ""
            if (
                plus_alleles
            ...
            if rsid in BASELINE_RSIDS:
                reverse_strand = normalized_strand in REVERSE_STRANDS
                if reverse_strand:
