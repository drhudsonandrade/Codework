    ...
    *,
    ...
    while size >= min_size:
        ...
        if len(lines) * leading <= max_height and all(
            pdfmetrics.stringWidth(line, font_name, size) <= max_width + 0.1 for line in lines
        ):
...
def _replacement_geometry(
    item: dict[str, Any], page_width: float, page_items: list[dict[str, Any]] | None = None
...
def render_pdf_from_template(
    rendered: dict[str, Any], path: Path, template_dir: Path, *, strict: bool = False
) -> dict[str, Any]:
    ...
    for page_no, page in enumerate(reader.pages, 1):
        ...
        for item in by_page_controls.get(page_no, []):
            ...
            c.rect(x0 - 0.9, height - y1 - 0.9, x1 - x0 + 1.8, y1 - y0 + 1.8, fill=1, stroke=0)
...
def _patch_docx_svg(docx_path: Path, svgs: list[Path]) -> None:
    ...
    try:
        ...
        if not any(element.get("Extension") == "svg" for element in root.findall(ct_ns + "Default")):
            element = etree.Element(ct_ns + "Default")
            element.set("Extension", "svg")
            element.set("ContentType", "image/svg+xml")
            root.append(element)
        tree.write(str(content_types), xml_declaration=True, encoding="UTF-8", standalone="yes")
        ...
        nums: list[int] = []
        for element in relationships.findall(rel_ns + "Relationship"):
            rid = element.get("Id", "")
        ...
        for i, src in enumerate(svgs, 1):
            name = f"genoma-page-{i}.svg"
            shutil.copy(src, media / name)
            rid = f"rId{next_id}"
            next_id += 1
            svg_rids.append(rid)
            element = etree.Element(rel_ns + "Relationship")
            element.set("Id", rid)
...
def render_docx_from_template(
    ...
    try:
        ...
        for page_no, png in enumerate(pngs, 1):
            ...
            inline = paragraph.add_run().add_picture(str(png), width=Mm(210), height=Mm(297))._inline
            ...
            for item in controls_by_page.get(page_no, []):
                ...
                bg = str(item["background"])
                fsize = float(value["font_size_pt"] or item.get("font_size_pt") or 7.0)
                fsize = _fit_single_line_size(
                    value["value"], max(8.0, bbox[2] - bbox[0]), fsize, _register_fonts()["bold"]
                )
                _add_vml_textbox(
                    paragraph,
            ...
            for item in fields_by_page.get(page_no, []):
                ...
                if raw is None:
                    ...
                    continue
                bg = str(item["background"])
                default = "#FFFFFF" if _is_dark(bg) else "#17212B"
                value = _normalize_value(raw, default_color=default)
                fsize = float(value["font_size_pt"] or item.get("font_size_pt") or 7.0)
                _cover, draw = _replacement_geometry(
                    item, float(meta["page_size_pt"][0]), fields_by_page.get(page_no, [])
                )
    ...
    if strict and unresolved:
        ...
        raise TemplateV3Error(f"strict v3 DOCX rendering refused: {len(unresolved)} unresolved fields")
    return {
...
def template_mode_requested(rendered: dict[str, Any]) -> bool:
    data = rendered.get("data") if isinstance(rendered.get("data"), dict) else {}
    return str(data.get("editorial_mode") or os.environ.get("GENOMA_EDITORIAL_MODE") or "").lower() in {
        "template-v3",
        "v3-template",
        "pixel-v3",
    }
