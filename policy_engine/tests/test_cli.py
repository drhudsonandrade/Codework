...
class CliTests(unittest.TestCase):
 ...
 def test_catalog_contains_all_rules(self):
  with tempfile.TemporaryDirectory() as td:
   out=Path(td)/"catalog.json"; r=self.run_cli("catalog","--output",str(out)); self.assertEqual(r.returncode,0,r.stderr); p=json.loads(out.read_text()); self.assertEqual(len(p["rules"]),263); self.assertEqual(p["rules"][0]["rule_id"],"GENOMA-V3.4-S000"); self.assertEqual(p["rules"][-1]["rule_id"],"GENOMA-V3.4-S262")
