...
class PathResolutionTests(unittest.TestCase):
    def setUp(self):
        self._ruleset_env = os.environ.pop("GENOMA_RULESET_PATH", None)
        self._manifest_env = os.environ.pop("GENOMA_RULESET_SHA_MANIFEST", None)
    ...
    def test_nested_engine_resolves_single_parent_ruleset(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            engine.mkdir()
            shutil.copyfile(RULESET, repo / RULESET.name)
            self.assertEqual(resolve_ruleset_path(engine), repo / RULESET.name)
    ...
    def test_duplicate_canonical_ruleset_locations_fail_closed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            engine.mkdir()
            shutil.copyfile(RULESET, repo / RULESET.name)
            shutil.copyfile(RULESET, engine / RULESET.name)
            with self.assertRaises(AssetResolutionError):
                resolve_ruleset_path(engine)
