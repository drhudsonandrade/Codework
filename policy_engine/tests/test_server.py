...
class ServerTests(unittest.TestCase):
 ...
 def test_health_and_ruleset(self):
  ...
  with urllib.request.urlopen(f"http://127.0.0.1:{self.port}/v1/ruleset",timeout=3) as r: p=json.load(r); self.assertEqual(p["version"],"v3.4"); self.assertEqual(p["effective_date"],"17/08/2026"); self.assertEqual(p["section_count"],263)
