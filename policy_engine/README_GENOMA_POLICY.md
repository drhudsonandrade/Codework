...
Deterministic, vendor-neutral executable policy for the canonical GENOMA v3.3 ruleset.
...
- 263 normative sections compile into stable rule IDs and section hashes.
...
- Canonical v3.3 bytes are activated from an inactive sealed transport only at runtime and mounted read-only.
...
- CLI and HTTP work with no external model, edge provider, orchestrator, database or hosted service dependency.
