...
class PolicyEngine(CoreGates, EvidenceGates, AuditGates):
    ...
    def evaluate(self, manifest: dict[str, Any]) -> EvaluationReport:
        ...
        report.gates.extend([
            ...
            self._negative_evidence_gate(manifest), self._ancestry_gate(manifest), self._reproductive_gate(manifest),
            self._pgx_complex_locus_gate(manifest), self._trait_determinism_gate(manifest), self._screening_diagnosis_gate(manifest),
        ...
        report.metadata = {
            "engine": "genoma-policy-engine", "engine_version": "0.2.0", "rule_count": len(self.ruleset.sections),
            "note": "POST-DEPLOYMENT is a distinct project gate and is never inferred from CI or local tests.",
        }
