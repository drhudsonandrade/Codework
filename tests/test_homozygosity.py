...
def _padding(count: int, *, chromosome: str = "9", start: int = 1):
    """Heterozygous filler, so the call-rate and density guards are satisfied."""
    return [(chromosome, start + i * 200, "AG") for i in range(count)]


class TractDetectionTest(unittest.TestCase):
    def test_a_long_dense_homozygous_run_is_a_tract(self):
        tracts = find_tracts(_run("1", 1_000_000, 400, 10_000, "AA"))
    ...
    def test_a_long_run_with_too_few_markers_is_not_a_tract(self):
        ...
        self.assertEqual(find_tracts(_run("1", 1_000_000, 20, 200_000, "AA")), [])

    def test_a_dense_run_that_is_too_short_is_not_a_tract(self):
        self.assertEqual(find_tracts(_run("1", 1_000_000, 300, 1_000, "AA")), [])
    ...
    def test_a_heterozygote_run_is_never_a_tract(self):
        self.assertEqual(find_tracts(_run("1", 1_000_000, 400, 10_000, "AG")), [])
    ...
    def test_one_heterozygote_is_tolerated_but_two_split_the_run(self):
        ...
        self.assertEqual(len(find_tracts(one)), 1)

        two = list(base)
        two[200] = (two[200][0], two[200][1], "AG")
        two[201] = (two[201][0], two[201][1], "AG")
        self.assertEqual(len(find_tracts(two)), 2)
...
class EstimateTest(unittest.TestCase):
    def _dense(self, homozygous_kb: float):
        ...
        """
        markers = _padding(MIN_CALLED_MARKERS, chromosome="9", start=120_000_000)
