...
readonly MANIFEST=${RULESET_MANIFEST:-"$PROJECT_ROOT/manifests/RULESET_V3.4.sha256"}
readonly CANONICAL_NAME=REGRAS_PROJETO_GENOMA_VIGENTE_v3.4_2026-08-17.txt
readonly RULESET_PATH=${1:?Usage: verify_ruleset.sh /secure/path/REGRAS_PROJETO_GENOMA_VIGENTE_v3.4_2026-08-17.txt}
...
grep -Fxq 'VERSÃO NORMATIVA: v3.4' "$RULESET_PATH"
grep -Fxq 'DATA FORMAL DE EMISSÃO E VIGÊNCIA: 17/08/2026' "$RULESET_PATH"
grep -Fxq 'IDENTIFICADOR NORMATIVO: GENOMA-RULESET-v3.4' "$RULESET_PATH"

printf 'ruleset_hash\tPASS\tsha256=%s\n' "$observed"
printf 'ruleset_header\tPASS\tstatus=VIGENTE\tversion=v3.4\tdate=17/08/2026\n'
