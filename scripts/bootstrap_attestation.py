#!/usr/bin/env python3
"""Fail-closed verifier and generator for the GENOMA v3.4 ruleset-bootstrap attestation.

The attestation is reproducible from the sealed canonical ruleset. Runtime witnesses
must additionally bind the generated artifact to the exact deployment Git commit, the
actual evidence locator and a SHA-256 captured immediately after generation. Digest
...
def _require_result_locator(value: Any) -> str:
    if not isinstance(value, str) or not value.strip() or not value.strip().endswith("#/checks"):
        raise BootstrapAttestationError(
            "bootstrap attestation result_locator must be a non-empty locator ending in #/checks"
        )
    return value.strip()


def _require_expected_file_sha256(value: Any) -> str:
    if value is None:
        raise BootstrapAttestationError("bootstrap attestation digest verification cannot be disabled")
    if not isinstance(value, str) or SHA256_RE.fullmatch(value) is None:
        raise BootstrapAttestationError(
            "bootstrap attestation expected_file_sha256 must be a 64-character lowercase SHA-256"
        )
    return value


def verify_project_bootstrap(
    sealed_dir: str | Path = SEALED_DIR,
    *,
    result_locator: str = RESULT_LOCATOR,
) -> dict[str, Any]:
    raw, transport_evidence = decode_verified_payload(Path(sealed_dir))
    text = raw.decode("utf-8")
    locator = _require_result_locator(result_locator)
    checks: dict[str, Any] = {}
...
def verify_bootstrap_attestation(
    ...
    *,
    ...
    expected_file_sha256: str | object = _DEFAULT_FILE_SHA,
    expected_result_locator: str | None = None,
) -> dict[str, Any]:
    source = Path(path)
    ...
    required_sha = (
        ...
        if expected_file_sha256 is _DEFAULT_FILE_SHA
        else _require_expected_file_sha256(expected_file_sha256)
    )
    if observed_sha != required_sha:
        raise BootstrapAttestationError(
    ...
    locator = RESULT_LOCATOR if expected_result_locator is None else _require_result_locator(expected_result_locator)
    try:
        evidence = verify_project_bootstrap(sealed_dir, result_locator=locator)
...
def build_attestation(
    *,
    ...
    evidence = verify_project_bootstrap(sealed_dir, result_locator=result_locator)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--result-locator", default=RESULT_LOCATOR)
    ...
    if args.write:
        ...
        payload = build_attestation(
            ...
            result_locator=args.result_locator,
        )
    ...
    evidence = verify_bootstrap_attestation(
        args.output,
        sealed_dir=args.sealed,
        expected_result_locator=args.result_locator,
    )
