...
Run ``python3 -m scripts.bootstrap_attestation --write`` to regenerate the attestation;
it is generated by execution and must never be edited by hand.
"""
...
EXPECTED_FILE_SHA256 = "3b0b062ff44710edfd91408452d31c7f7baaeb40f377a23a6146e6bb38f28f8f"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write:
        existing = json.loads(Path(args.output).read_text(encoding="utf-8")) if Path(args.output).is_file() else {}
        verified_at = args.verified_at or existing.get("verified_at")
        if not verified_at:
            parser.error("--verified-at is required when no previous attestation exists")
        payload = build_attestation(sealed_dir=args.sealed, verified_at=str(verified_at))
